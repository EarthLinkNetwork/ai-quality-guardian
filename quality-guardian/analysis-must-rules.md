...
**このセクションは pre-commit hook で自動チェックされます。**
「実装可能」と判定したルールは、全て実装する必要があります。
未実装（`[ ]`）が残っている場合、コミットはブロックされます。

### 実装必須ルール

- [x] Rule 17: Claude Code痕跡排除（v1.3.59で実装済み）
- [x] Rule 4: Git操作前の確認（v1.3.60で実装 - プロジェクト判定機能付き）
- [x] Rule 13: Git操作方法（v1.3.60で実装 - Rule 4に統合）
- [x] Rule 16: テンプレート同期（v1.3.60で実装 - pre-commit hookで差分検証）
- [x] Rule 20: PR状態確認（v1.3.60で実装 - post-commit hookで状態確認）

## 次のステップ

1. Rule 17（Claude Code痕跡排除）のESLintルール実装 ✅ 完了（v1.3.59）
2. Rule 4（Git操作前の確認）のpre-commit hook強化 ✅ 完了（v1.3.60）
3. Rule 16（テンプレート同期）のpre-commit hook追加 ✅ 完了（v1.3.60）
4. Rule 13（Git操作方法）のGit設定 ✅ 完了（v1.3.60 - Rule 4に統合）
5. Rule 20（PR状態確認）のpost-commitスクリプト ✅ 完了（v1.3.60）
6. バージョン更新（v1.3.60） → 次の実施項目

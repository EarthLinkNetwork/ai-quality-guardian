...
exports.SELFTEST_TASK_GROUP = exports.SELFTEST_CASES = exports.SELFTEST_TASK_TYPE = exports.SELFTEST_TASK_GROUP_PREFIX = void 0;
exports.generateSelftestRunId = generateSelftestRunId;
exports.generateSelftestSessionId = generateSelftestSessionId;
exports.runSelftestWithAIJudge = runSelftestWithAIJudge;
...
const mock_executor_1 = require("./mock-executor");
/**
 * Generate a unique selftest run ID
 * Format: selftest-YYYYMMDD-HHMM-<random>
 */
function generateSelftestRunId() {
    const now = new Date();
 ...
 */
function generateSelftestSessionId(runId) {
    return `session-${runId}`;
}
/**
 * Selftest task group prefix
 */
exports.SELFTEST_TASK_GROUP_PREFIX = 'tg_selftest_';
 ...
 */
async function waitForTaskCompletion(queueStore, taskId, timeoutMs = 60000, pollIntervalMs = 1000) {
 ...
 */
async function executeSelftestCase(queueStore, sessionId, taskGroupId, generatedPrompt, judge, config, scenario, executor) {
    const startTime = Date.now();
    // Enqueue the test task
    const task = await queueStore.enqueue(sessionId, taskGroupId, generatedPrompt.prompt, undefined, exports.SELFTEST_TASK_TYPE);
 ...
 */
function buildReport(runId, config, cases) {
    ...
    return {
        ...
        config: {
            ...
            effective_threshold: (0, config_loader_1.calculateEffectiveThreshold)(config),
        },
        summary: {
            total: cases.length,
            passed,
 ...
 */
function writeJsonReport(report, baseDir) {
 ...
 */
function writeMarkdownReport(report, baseDir) {
 ...
 */
async function runSelftestWithAIJudge(queueStore, options) {
    console.log('[selftest] Starting selftest with AI Judge...');
    console.log(`[selftest] Mode: ${options.ci ? 'CI (short)' : 'Full'}`);
    // Load configuration
    let config = (0, config_loader_1.loadSelftestConfig)(options.configPath);
    // Filter scenarios for CI mode
    if (options.ci) {
        config = (0, config_loader_1.filterScenariosForCI)(config);
    ...
    const taskGroupId = `${exports.SELFTEST_TASK_GROUP_PREFIX}${runId}`;
    ...
    const generator = (0, generator_1.createGenerator)(config.generator);
    const judge = (0, judge_1.createJudge)(config.judge);
    const executor = (0, mock_executor_1.createMockExecutor)(queueStore);
    ...
    for (const scenario of config.scenarios) {
        ...
        const result = await executeSelftestCase(queueStore, sessionId, taskGroupId, generatedPrompt, judge, config, scenario, executor);

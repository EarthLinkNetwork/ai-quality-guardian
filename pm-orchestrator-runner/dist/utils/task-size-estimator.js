...
exports.getCustomProfileForTask = getCustomProfileForTask;
exports.quickEstimateProfile = quickEstimateProfile;
const timeout_profile_1 = require("./timeout-profile");
/**
 ...
 */
function estimateTaskSize(prompt, taskType) {
    const factors = [];
    let totalScore = 0;
 ...
 */
function categorizeScore(score, factors) {
    // Score ranges:
    // < -0.3: small
    // -0.3 to 0.2: medium
    // 0.2 to 0.6: large
    ...
    if (score < -0.3) {
        category = 'small';
        profile = timeout_profile_1.STANDARD_PROFILE;
        ...
        category = 'large';
        profile = timeout_profile_1.LONG_PROFILE;
        explanation = 'Complex task - using long timeout profile';
    }
    else {
        category = 'x-large';
        profile = timeout_profile_1.EXTENDED_PROFILE;
 ...
 */
function getCustomProfileForTask(estimate, overrides) {
    const baseProfile = estimate.recommendedProfile;
    if (!overrides?.idle_timeout_ms && !overrides?.hard_timeout_ms) {
        return baseProfile;
    }
    return (0, timeout_profile_1.createCustomProfile)(overrides.idle_timeout_ms ?? baseProfile.idle_timeout_ms, overrides.hard_timeout_ms ?? baseProfile.hard_timeout_ms);
}
/**
 ...
 */
function quickEstimateProfile(prompt, taskType) {
    const estimate = estimateTaskSize(prompt, taskType);
    return estimate.recommendedProfile;
}
//# sourceMappingURL=task-size-estimator.js.map
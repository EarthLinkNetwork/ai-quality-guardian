...
class RunnerCore extends events_1.EventEmitter {
    ...
    async initialize(targetProject) {
        ...
        if (this.options.useClaudeCode) {
            ...
            (0, recovery_executor_1.assertRecoveryModeAllowed)();
            ...
            if (testMode !== 'passthrough' && this.claudeCodeExecutor) {
                this.claudeCodeExecutor = (0, test_incomplete_executor_1.wrapWithTestExecutor)(this.claudeCodeExecutor);
    ...
    async executeTask(task) {
        ...
        if (this.taskLogManager && this.session && this.taskLogThread && this.taskLogRun) {
            taskLog = await this.taskLogManager.createTaskWithContext(this.session.session_id, this.taskLogThread.thread_id, this.taskLogRun.run_id, undefined, // parentTaskId
                ...
                if (this.claudeCodeExecutor && this.session?.target_project) {
                    ...
                    if (executorResult.status === 'NO_EVIDENCE') {
                        ...
                        if ((task.taskType === 'READ_INFO' || task.taskType === 'REPORT') && executorResult.output) {
                            ...
                            if (taskLog && this.taskLogManager && this.session) {
                                await this.taskLogManager.completeTaskWithSession(taskLog.task_id, this.session.session_id, 'COMPLETE', filesCreated, undefined, undefined, executorBlockingInfo.executor_blocked !== undefined ? {
                    ...
                    if (executorResult.status === 'INCOMPLETE') {
                        if (task.taskType === 'READ_INFO' || task.taskType === 'REPORT') {
                            if (executorResult.output && executorResult.output.trim().length > 0) {
                                ...
                                result.status = enums_1.TaskStatus.INCOMPLETE;
                                result.clarification_needed = true;
                                result.clarification_reason = 'SCOPE_UNCLEAR';
                                ...
                                if (taskLog && this.taskLogManager && this.session) {
                                    await this.taskLogManager.completeTaskWithSession(taskLog.task_id, this.session.session_id, 'INCOMPLETE', filesCreated, undefined, `clarification_required:SCOPE_UNCLEAR`, executorBlockingInfo.executor_blocked !== undefined ? {
                                        executorBlocked: executorBlockingInfo.executor_blocked,
                                        blockedReason: executorBlockingInfo.blocked_reason,
                                        timeoutMs: executorBlockingInfo.timeout_ms,
                                        terminatedBy: executorBlockingInfo.terminated_by,
                        ...
                        if (taskLog && this.taskLogManager && this.session) {
                            await this.taskLogManager.completeTaskWithSession(taskLog.task_id, this.session.session_id, 'ERROR', filesCreated, undefined, 'INCOMPLETE status: no evidence of work', executorBlockingInfo.executor_blocked !== undefined ? {
                                executorBlocked: executorBlockingInfo.executor_blocked,
                                blockedReason: executorBlockingInfo.blocked_reason,
                                timeoutMs: executorBlockingInfo.timeout_ms,
                                terminatedBy: executorBlockingInfo.terminated_by,
     ...
     */
    generateClarificationForIncomplete(task) {

 ...
 */
export class FileLockValidationError extends Error {
 ...
 */
export function validateFileLock(lock: FileLock): boolean {
  ...
  // Validate lock_type
  if (!Object.values(LockType).includes(lock.lock_type)) {
 ...
 */
export function isLockHeldBy(lock: FileLock, executorId: string): boolean {
 ...
 */
export function isLockCompatible(existingLock: FileLock, requestedType: LockType): boolean {
  // WRITE lock is exclusive
  if (existingLock.lock_type === LockType.WRITE) {
    return false;
  }

  // READ lock only allows other READ locks
  if (existingLock.lock_type === LockType.READ) {
    return requestedType === LockType.READ;
  }
 ...
 */
export function extendLockExpiration(
  lock: FileLock,
 ...
 */
export function isExpired(lock: FileLock): boolean {
  const expiresAt = new Date(lock.expires_at);

...
export class TemplateCommand {
   ...
   */
  private ensureStore(): TemplateStore {
   ...
   */
  private ensureSettings(): ProjectSettingsStore {
   ...
   * List all templates
   */
  async list(): Promise<TemplateResult> {
    try {
   ...
   */
  async create(name: string, rulesText: string, outputFormatText: string): Promise<TemplateResult> {
    try {
      const store = this.ensureStore();
  ...
  async update(
    id: string,
    updates: { name?: string; rulesText?: string; outputFormatText?: string }
    ...
    try {
      ...
      if (updates.name && updates.name !== existing.name) {
        const validation = validateTemplateName(updates.name);
        if (!validation.valid) {
          return {
            success: false,
            error: {
   ...
   * Delete a template
   */
  async delete(nameOrId: string): Promise<TemplateResult> {
    try {
      const store = this.ensureStore();
   ...
   */
  async copy(sourceNameOrId: string, newName: string): Promise<TemplateResult> {
    try {
      const store = this.ensureStore();
   ...
   */
  async use(nameOrId: string): Promise<TemplateResult> {
    try {
      const store = this.ensureStore();
   ...
   */
  async enable(): Promise<TemplateResult> {
    try {
      const settings = this.ensureSettings();
   ...
   */
  async disable(): Promise<TemplateResult> {
    try {
      const settings = this.ensureSettings();
   ...
   */
  async show(): Promise<TemplateResult> {
    try {
      const settings = this.ensureSettings();
      const store = this.ensureStore();

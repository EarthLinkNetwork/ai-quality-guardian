...
const ANSI = {
  ...
  MOVE_TO_COL: (col: number) => `\x1b[${col}G`,
  HIDE_CURSOR: '\x1b[?25l',
  SHOW_CURSOR: '\x1b[?25h',
  BOLD: '\x1b[1m',
  RESET: '\x1b[0m',
 ...
 */
export class InteractivePicker<T = unknown> {
  private items: PickerItem<T>[];
  private options: InteractivePickerOptions;
   ...
   */
  private getVisibleWindow(): { items: PickerItem<T>[]; startIndex: number } {
   ...
   */
  private render(isInitial: boolean = false): void {
   ...
   */
  private promptNonInteractive(): Promise<PickerSelection<T>> {
    const lines: string[] = [];

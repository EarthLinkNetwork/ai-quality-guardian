...
// ============================================================================
// Type Definitions (per spec/29_TASK_PLANNING.md Section 3)
// ============================================================================
 ...
 */
export type PlanningEventCallback = (
  eventType: string,
 ...
 */
export const DEFAULT_TASK_PLANNER_CONFIG: TaskPlannerConfig = {
  auto_chunk: true,
...
const FILE_COUNT_INDICATORS = [
  { pattern: /\b(\d+)\s*(?:files?|components?|modules?)/i, extractor: (m: RegExpMatchArray) => parseInt(m[1], 10) },
 ...
 */
export function estimateTaskSize(prompt: string): SizeEstimation {
 ...
 */
export function determineChunking(
  prompt: string,
  sizeEstimation: SizeEstimation,
  config: TaskPlannerConfig = DEFAULT_TASK_PLANNER_CONFIG
 ...
 */
function extractSubtasks(prompt: string, config: TaskPlannerConfig): PlanningSubtask[] {
  const subtasks: PlanningSubtask[] = [];
  let order = 1;
 ...
 */
export function analyzeDependencies(subtasks: PlanningSubtask[]): DependencyAnalysis {
 ...
 */
function detectImplicitDependency(task1: PlanningSubtask, task2: PlanningSubtask): boolean {
 ...
 */
function findParallelizableGroups(
  subtasks: PlanningSubtask[],
  edges: DependencyEdge[]
): string[][] {
  const groups: string[][] = [];
...
export function generateExecutionPlan(
  ...
  let executionStrategy: ExecutionPlan['execution_strategy'] = 'single';
...
export class TaskPlanner {
  private readonly config: TaskPlannerConfig;
  private readonly eventCallback?: PlanningEventCallback;
  private readonly conversationTracer?: ConversationTracer;

  constructor(
    config: Partial<TaskPlannerConfig> = {},
    eventCallback?: PlanningEventCallback,
    conversationTracer?: ConversationTracer
  ) {
    this.config = { ...DEFAULT_TASK_PLANNER_CONFIG, ...config };
  ...
  plan(taskId: string, prompt: string): ExecutionPlan {
    ...
    if (this.conversationTracer && plan.chunking_recommendation.should_chunk) {
      const subtaskPlans: SubtaskPlan[] = plan.chunking_recommendation.subtasks.map(s => ({
        id: s.id,

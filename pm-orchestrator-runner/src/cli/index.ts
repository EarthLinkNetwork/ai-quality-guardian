    ...
    try {
      ...
      if (cleanOutput && cleanOutput.trim().length > 0 && result.status !== 'ERROR') {
        try {
          ...
          if (claimCheck.hasUnverifiedClaims) {
            ...
            log.app.warn('Unverified claims detected', { taskId: item.task_id, claimCount: claimCheck.claims.length });
      ...
      if (result.status === 'COMPLETE') {
        ...
        if (checkpoint && checkpoint.type !== 'none') {
          ...
          log.app.warn('Rollback executed', { taskId: item.task_id, checkpointType: checkpoint.type });

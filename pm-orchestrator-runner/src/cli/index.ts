...
async function recordTaskLlmCost(item: QueueItem): Promise<void> {
  ...
  if (totalCostUsd > 0 || taskUsage.length > 0) {
    log.app.info('LLM usage recorded', { calls: taskUsage.length, costUsd: totalCostUsd.toFixed(4) });
  }
...
function createTaskExecutor(projectPath: string, queueStore: IQueueStore): TaskExecutor {
  return async (item: QueueItem): Promise<{ status: 'COMPLETE' | 'ERROR' | 'WAITING_CHILDREN'; errorMessage?: string; output?: string }> => {
    ...
    try {
    log.app.info('Executing task', { taskId: item.task_id, promptPreview: item.prompt.substring(0, 100) });
    ...
    if (item.conversation_history && item.conversation_history.length > 0) {
      ...
      log.app.info('Re-executing with conversation history', { historyEntries: item.conversation_history.length, hasOutput: !!item.output, hasClarification: !!item.clarification?.question });
      ...
      try {
        ...
        log.sys.warn('Failed to write conversation history to trace', { error: (traceErr as Error).message });
      }
    }
    ...
    const enqueuePipelineSubtasks = async (parentPrompt: string, parentOutput: string | undefined): Promise<string[]> => {
      ...
      if (item.add_review === true) {
        try {
          ...
          log.app.info('Review subtask enqueued', { taskId: reviewTaskId, parentTaskId: item.task_id });
          stateStream.emit(item.task_id, 'system', `[pipeline] Review subtask enqueued: ${reviewTaskId}`);
        } catch (pipeErr) {
          log.sys.warn('Failed to enqueue review subtask', { error: String(pipeErr) });
        }
      }
      ...
      if (item.add_test === true) {
        try {
          ...
          log.app.info('Test subtask enqueued', { taskId: testTaskId, parentTaskId: item.task_id });
          stateStream.emit(item.task_id, 'system', `[pipeline] Test subtask enqueued: ${testTaskId}`);
        } catch (pipeErr) {
          log.sys.warn('Failed to enqueue test subtask', { error: String(pipeErr) });
        }
      }
      return enqueuedIds;
    };
    ...
    if (isInitialExecution && !isSubtask) {
      ...
      try {
        ...
        if (metaResult.usedProvider && metaResult.metaPrompt !== effectivePrompt) {
          ...
          metaPromptUsed = true;
          log.app.info('Meta prompt generated', { provider: metaResult.usedProvider, taskId: item.task_id, enhancements: metaResult.enhancements });
          stateStream.emit(item.task_id, 'system', `[llm-relay] Meta prompt generated (${metaResult.usedProvider}): ${metaResult.enhancements}`);
          writeLlmTraceEvent({ type: 'llm_processing', action: 'meta_prompt_done', provider: metaResult.usedProvider, enhancements: metaResult.enhancements, metaPrompt: promptForClaude, originalPrompt: effectivePrompt });
        } else {
        ...
        if (item.add_test || item.add_review) {
          if (metaResult.shouldSplit) {
            log.app.info('LLM split suppressed for pipeline', { taskId: item.task_id, addTest: item.add_test, addReview: item.add_review });
        ...
        log.sys.warn('Meta prompt generation failed, using raw prompt', { error: String(metaErr) });
    ...
    stateStream.emit(
      ...
      if (!item.conversation_history?.length && !item.parent_task_id) {
        ...
        if (llmSplitResult?.shouldSplit && llmSplitResult.subtasks && llmSplitResult.subtasks.length >= 2) {
          try {
            log.app.info('Using LLM split', { taskId: item.task_id, reason: llmSplitResult.splitReason });
            ...
            log.sys.warn('LLM split enqueue failed, falling back to regex', { error: String(llmSplitErr) });
          }
        }
        ...
        if (!decomposed) {
          try {
            ...
            log.sys.warn('Task decomposition check failed, proceeding with single execution', { error: String(decompErr) });
          }
        }
      }
    ...
    if (!item.parent_task_id) {
      ...
      try {
        ...
        if (checkpoint && checkpoint.type !== 'none') {
          ...
          try {
            ...
            log.sys.warn('Failed to persist checkpoint_ref', { error: String(persistErr) });
          }
        }
      } catch (cpErr) {
        log.sys.warn('Checkpoint creation failed, proceeding without rollback safety', { error: String(cpErr) });
      }
    } else {
      ...
      try {
        ...
        log.sys.warn('Failed to load parent checkpoint', { error: String(parentErr) });
      }
    }
    ...
    try {
      ...
      if (testMode !== 'passthrough') {
        log.app.info('Test executor mode', { mode: testMode });
        ...
        log.app.info('Test executor returned', { status: result.status });
        ...
        if (hasOutput) {
          try {
            ...
            log.app.info('LLM question detection', { hasQuestions: llmResult.hasQuestions, summary: llmResult.questionSummary });
          } catch (e) {
            log.sys.warn('LLM detection failed, using regex fallback');
          }
        }
        ...
        if (result.status === 'COMPLETE') {
          ...
          if (hasQuestions) {
            log.app.info('COMPLETE but has questions, transitioning to AWAITING_RESPONSE', { taskId: item.task_id });
            return {
              status: 'ERROR',
          ...
          if (hasOutput) {
            if (hasQuestions) {
              log.app.info('READ_INFO/REPORT with questions, transitioning to AWAITING_RESPONSE', { taskId: item.task_id, status: result.status });
              return {
                status: 'ERROR',
                errorMessage: 'AWAITING_CLARIFICATION:' + questionSummary,
                output: cleanOutput,
            ...
            log.app.info('READ_INFO/REPORT with output, transitioning to COMPLETE', { taskId: item.task_id, status: result.status });
            return { status: 'COMPLETE', output: cleanOutput };
          } else {
            log.app.info('READ_INFO/REPORT without output, needs clarification', { taskId: item.task_id, status: result.status });
            return {
              status: 'ERROR',
          ...
          if (hasQuestions) {
            log.app.info('Task with questions, transitioning to AWAITING_RESPONSE', { taskId: item.task_id, taskType, status: result.status });
            return {
              status: 'ERROR',
              errorMessage: 'AWAITING_CLARIFICATION:' + questionSummary,
              output: cleanOutput,
      ...
      log.app.info('Timeout profile', { taskId: item.task_id, profile: timeoutProfile.name, idleMs: timeoutProfile.idle_timeout_ms, hardMs: timeoutProfile.hard_timeout_ms, disableOverall: disableOverallTimeout });
      stateStream.emit(
        item.task_id,
        'timeout',
      ...
      log.app.info('Task execution completed', { taskId: item.task_id, status: result.status });
      ...
      if (cleanOutput && cleanOutput.trim().length > 0 && result.status !== 'ERROR') {
        try {
          ...
          if (!qaResult.passed && qaResult.reworkInstructions) {
            log.app.info('QA failed, re-executing with rework', { taskId: item.task_id, issues: qaResult.issues });
            ...
            if (reworkClean && reworkClean.trim().length > 0) {
              log.app.info('Rework produced output', { taskId: item.task_id, outputLen: reworkClean.length });
              ...
              log.app.info('Rework produced no output, keeping original', { taskId: item.task_id });
              stateStream.emit(item.task_id, 'system', `[llm-relay] Rework produced no output, keeping original`);
            }
          } else if (qaResult.passed) {
            log.app.info('QA passed', { taskId: item.task_id });
          ...
          log.sys.warn('QA evaluation failed, skipping', { error: qaErr instanceof Error ? qaErr.message : String(qaErr) });
        }
      }
      ...
      if (cleanOutput && cleanOutput.trim().length > 0 && result.status !== 'ERROR') {
        try {
          ...
          if (claimCheck.hasUnverifiedClaims) {
            ...
            log.app.warn('Unverified claims detected', { taskId: item.task_id, claimCount: claimCheck.claims.length, claims: claimWarnings });
      ...
      if (hasOutput) {
        try {
          ...
          llmResult = await detectQuestionsWithLlm(cleanOutput, item.prompt, undefined, undefined);
          log.app.info('LLM question detection', { taskId: item.task_id, hasQuestions: llmResult.hasQuestions, summary: llmResult.questionSummary });
          ...
          log.sys.warn('LLM detection failed, using regex fallback', { taskId: item.task_id });
        }
      }
      const hasQuestions = llmResult ? llmResult.hasQuestions : (hasOutput && hasUnansweredQuestions(cleanOutput));
      const questionSummary = llmResult?.questionSummary || (hasQuestions ? extractQuestionSummary(cleanOutput) : '');
      ...
      if (hasQuestions && questionSummary) {
        try {
          log.app.info('Questions detected, attempting auto-answer', { taskId: item.task_id });
          ...
          if (autoAnswer.canAnswer && autoAnswer.answer) {
            log.app.info('Auto-answer resolved', { taskId: item.task_id, answerLength: autoAnswer.answer.length });
            ...
            if (reClean && reClean.trim().length > 0) {
              ...
              if (retryHasQuestions) {
                log.app.info('Auto-answer retry still has questions, transitioning to AWAITING_RESPONSE', { taskId: item.task_id });
              ...
              log.app.info('Auto-answer retry succeeded', { taskId: item.task_id });
              ...
              log.app.info('Auto-answer retry produced no output, falling through', { taskId: item.task_id });
            }
          } else {
            log.app.info('Auto-answer cannot resolve, escalating to user', { taskId: item.task_id, reasoning: autoAnswer.reasoning });
          }
        } catch (autoErr) {
          log.sys.warn('Auto-answer attempt failed', { taskId: item.task_id, error: autoErr instanceof Error ? autoErr.message : String(autoErr) });
        }
      }
      ...
      if (result.status === 'COMPLETE') {
        ...
        if (cleanOutput) {
          ...
          if (redCheck.detected) {
            log.app.warn('RED operation detected', { taskId: item.task_id, operations: redCheck.operations });
        ...
        // Rollback on failure
        if (checkpoint && checkpoint.type !== 'none') {
          log.app.warn('Rolling back checkpoint for failed task', { taskId: item.task_id, checkpointType: checkpoint.type });
          ...
          if (rbResult.success) {
            ...
            log.sys.warn('Rollback failed', { taskId: item.task_id, error: rbResult.error });
          }
        }
        ...
        if (hasOutput) {
          ...
          log.app.info('READ_INFO/REPORT without output, transitioning to AWAITING_RESPONSE', { taskId: item.task_id, status: result.status });
        ...
        if (result.executed && hasOutput) {
          log.app.info('Task with exit=0 and output, file verification override to COMPLETE', { taskId: item.task_id, taskType, status: result.status });
        ...
        log.app.info('Task transitioning to ERROR', { taskId: item.task_id, taskType, status: result.status, hasOutput, outputLen: cleanOutput?.length ?? 0 });
      ...
      log.sys.error('Task failed with exception', { taskId: item.task_id, error: errorMessage });
      ...
      if (checkpoint && checkpoint.type !== 'none') {
        ...
        try {
          ...
          log.sys.warn('Rollback failed with exception', { taskId: item.task_id, error: String(rbErr) });
        }
      }
...
async function startWebServer(webArgs: WebArguments): Promise<void> {
  ...
  if (storeMode === 'memory') {
    ...
    try {
      ...
      log.sys.info('DynamoDB connected', { table: 'pm-runner-queue', endpoint: dynamoStore.getEndpoint(), local: useLocalDynamodb });
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
  ...
  if (preflightReport.can_proceed) {
    ...
    log.sys.info('Preflight passed', { checks: preflightReport.checks.filter(c => c.ok).length });
    console.log('');
  } else if (allowPreflightFailure) {
    console.warn('[Preflight] Executor preflight failed; Web UI running in limited mode.');
    log.sys.warn('Preflight failed', { errors: preflightReport.fatal_errors.map(e => e.code) });
    console.warn('');
  }
  ...
  const runnerRestartHandler = async () => {
    ...
    try {
      ...
      try {
        ...
        if (fs.existsSync(publicHtmlSrc) && fs.existsSync(publicHtmlDist)) {
          ...
          if (srcSize !== distSize) {
            ...
            log.sys.info('index.html size mismatch fixed', { srcSize, distSize });
          }
          log.sys.info('Public assets copied', { srcSize, distSize });
        }
      } catch (e) {
        log.sys.warn('Could not copy public assets after build', { error: String(e) });
      }
      ...
      try {
        ...
        log.sys.warn('Could not read build-meta.json', { error: String(e) });
      }

      const output = (stdout || '') + (stderr ? '\n' + stderr : '');

      const postResponse = async () => {
        log.sys.info('Build & Restart initiated', { buildSha: buildMeta?.build_sha });
        ...
        try {
          ...
          log.sys.warn('Failed to stop poller during restart', { error: String(error) });
        }

        try {
          await serverRef?.stop();
        } catch (error) {
          log.sys.warn('Failed to stop server during restart', { error: String(error) });
        }
          ...
          for (const line of lines) {
            ...
            if (pid && pid !== currentPid) {
              try {
                ...
                log.sys.info('Killed stale process', { pid });
        ...
        // Re-read .env file so new env vars (like ORG_ID) are picked up on restart
        const freshEnv: Record<string, string | undefined> = { ...process.env };
        try {
        ...
        if (process.env.PM_RUNNER_BACKGROUND === '1' && child.pid) {
          try {
            ...
            log.sys.warn('Failed to update PID file after restart', { error: String(error) });
          }
        }
      };
      ...
      return {
        ...
        postResponse: () => {
          setTimeout(() => {
            postResponse()
              .catch((error: unknown) => {
                log.sys.error('Self-restart failed', { error: String(error) });
              })
              .finally(() => {
                process.exit(0);
              });
  ...
  const updateConversationFromTask = async (item: QueueItem, status: 'complete' | 'error', errorMessage?: string) => {
    try {
      ...
      if (!run) {
        log.sys.warn('No run found, skipping conversation update', { taskRunId: rawTaskRunId });
        return;
      }
      ...
      if (!assistantMsg) {
        log.sys.warn('No assistant message found, skipping conversation update', { runId: run.runId });
        return;
      }
      ...
      log.app.info('Updated conversation message', { runId: run.runId, status: isDecomposedTask ? 'processing (decomposed)' : status });
    } catch (err) {
      log.sys.error('Failed to update conversation message', { error: String(err) });
    }
  };
    ...
    try {
      ...
      if (anyAwaiting && parentItem && parentItem.status === 'WAITING_CHILDREN') {
        try {
          ...
          log.app.info('Parent transitioning to AWAITING_RESPONSE (child awaiting)', { parentTaskId: item.parent_task_id });
        } catch (transitionErr) {
          log.sys.warn('Failed to propagate AWAITING to parent', { parentTaskId: item.parent_task_id, error: String(transitionErr) });
        }
      }
      ...
      if (parentItem) {
        try {
          ...
          log.app.info('Parent task status updated', { parentTaskId: item.parent_task_id, status: parentFinalStatus, subtaskCount: siblings.length });
        } catch (parentUpdateErr) {
          log.sys.warn('Failed to update parent queue task status', { parentTaskId: item.parent_task_id, error: String(parentUpdateErr) });
        }
      }
      ...
      log.app.info('Aggregated parent task conversation', { parentTaskId: item.parent_task_id, status: parentFinalStatus });
    } catch (err) {
      log.sys.error('Failed to aggregate parent conversation', { parentTaskId: item.parent_task_id, error: String(err) });
    }
  };

  // Set up poller event listeners
  poller.on('started', () => {
    log.sys.info('Queue poller started', { runnerId: poller.getRunnerId() });
  });
  ...
  poller.on('stale-recovered', (count: number) => {
    log.sys.warn('Stale tasks recovered', { count });
  });
  ...
  if (authRequested) {
    ...
    try {
      ...
      if (webArgs.apiKey) {
        ...
        log.sys.info('Auth configured', { userId: keyData.userId, deviceName: keyData.deviceName });
      } else {
        console.log('[Auth] Authentication enabled. Users must provide API key via WebUI login.');
        log.sys.info('Auth configured', { mode: 'webui-login' });
      }
  ...
  if (!preflightReport.can_proceed && allowPreflightFailure) {
    log.sys.warn('Queue poller not started because preflight failed');
    log.sys.warn('Configure an executor in Settings, then restart to enable task processing');
  } else {
    await poller.start();
  }
  ...
  log.sys.info('Web server and queue poller are running');
  log.sys.info('Press Ctrl+C to stop');

  // Handle graceful shutdown
  const shutdown = async () => {
    log.sys.info('Shutting down...');
    detachProgressPersistence();
    await poller.stop();
    await server.stop();
...
async function startAgentOnly(agentArgs: AgentArguments): Promise<void> {
  ...
  try {
    ...
    queueStore = dynamoStore;
    if (useLocalDynamodb) {
      log.sys.info('Agent using DynamoDB Local', { endpoint: dynamoStore.getEndpoint() });
    } else {
      log.sys.info('Agent using AWS DynamoDB', { profile: getAwsProfile(), endpoint: dynamoStore.getEndpoint() });
    }
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    log.sys.error('Agent failed to connect to DynamoDB', { error: errorMessage });
    log.sys.error('Agent mode requires DynamoDB. Use --local-dynamodb for local development');
    process.exit(1);
  }
  ...
  if (agentArgs.apiKey) {
    ...
    try {
      ...
      if (!keyData) {
        log.sys.error('Agent invalid API key', { keyPrefix: agentArgs.apiKey.substring(0, 8) });
        process.exit(1);
      }
      log.sys.info('Agent authenticated', { userId: keyData.userId, deviceName: keyData.deviceName });
    } catch (error) {
      log.sys.warn('Agent API key validation failed', { error: error instanceof Error ? error.message : String(error) });
    }
  }
  ...
  const preflightReport = runPreflightChecks('auto');
  if (!preflightReport.can_proceed) {
    log.sys.error('Agent executor preflight failed. No executor available');
    for (const err of preflightReport.fatal_errors) {
      console.error(`  [${err.code}] ${err.message}`);
    }
    process.exit(1);
  ...
  poller.on('started', () => log.sys.info('Agent queue poller started'));
  poller.on('claimed', (item: QueueItem) => log.app.info('Agent claimed task', { taskId: item.task_id }));
  poller.on('completed', (item: QueueItem) => log.app.info('Agent completed task', { taskId: item.task_id }));
  poller.on('error', (item: QueueItem, error: Error) => log.app.error('Agent task error', { taskId: item.task_id, error: error.message }));

  log.sys.info('Starting agent-only mode', { namespace: namespaceConfig.namespace, projectPath, autoUpdate: !!agentArgs.autoUpdate });
  console.log('');
  ...
  // Graceful shutdown
  const shutdown = async () => {
    log.sys.info('Agent shutting down...');
    stopAutoUpdate?.();
    detachPersistence();
    await poller.stop();
    log.sys.info('Agent shutdown complete');
    process.exit(0);
  };

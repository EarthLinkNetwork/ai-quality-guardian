...
export function maskSensitiveObject<T>(obj: T): T {
  ...
  if (Array.isArray(obj)) {
    return obj.map(item => maskSensitiveObject(item)) as unknown as T;
  }
 ...
 */
export function getApiKeyStatus(envVar: string): 'SET' | 'NOT SET' {
 ...
 */
export function checkApiKeyForProvider(provider: string): {
  required: boolean;
  envVar: string | null;
  status: 'SET' | 'NOT SET' | 'NOT_REQUIRED';
} {
  const keyMapping: Record<string, string | null> = {
    'claude-code': null,
 ...
 */
export function getPatternsByPriority(priority: 1 | 2 | 3 | 4): string[] {

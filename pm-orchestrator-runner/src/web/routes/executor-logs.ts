...
} from '../../executor/executor-output-stream';
 ...
 */
export function createExecutorLogsRoutes(): Router {
  const router = Router();
  ...
  router.get('/logs', (req: Request, res: Response) => {
    ...
    let chunks: ExecutorOutputChunk[];

    // Apply filters - use stale-aware filtering when taskId provided
    if (taskId && typeof taskId === 'string') {
      chunks = outputStream.getByTaskIdFiltered(
        taskId,
        typeof taskCreatedAt === 'string' ? taskCreatedAt : undefined
      );
  ...
  router.get('/logs/task/:taskId', (req: Request, res: Response) => {
    ...
    const taskCreatedAt = req.query.taskCreatedAt as string | undefined;
    const limit = req.query.limit ? parseInt(req.query.limit as string, 10) : 100;
  ...
  router.get('/logs/stream', (req: Request, res: Response) => {
    ...
    const sinceSequence = req.query.since ? parseInt(req.query.since as string, 10) : undefined;
    ...
    if (sinceSequence !== undefined && !isNaN(sinceSequence)) {
      ...
      for (const chunk of missedChunks) {
        if (taskId && chunk.taskId !== taskId) continue;
    ...
    const subscriber: ExecutorOutputSubscriber = {
      onOutput(chunk: ExecutorOutputChunk) {
        ...
        if (taskId && isStaleNotification(chunk, {

 ...
 */
function getDefaultOutput(expectedStatus: string): { output: string; status: string } {
 ...
 */
export function getMockOutput(scenarioId: string, expectedStatus: string): { output: string; status: string } {
 ...
 */
export class MockExecutor implements IMockExecutor {
  constructor(private queueStore: IQueueStore) {}

  /**
   * Process a single task
   * Updates status and output based on scenario expectations
   */
  async processTask(taskId: string, scenario: SelftestScenario): Promise<QueueItem> {
    // Get the task
    ...
    await this.queueStore.updateStatus(
      taskId,
      'RUNNING' as any,
    ...
    await this.queueStore.updateStatus(
      taskId,
      mockResult.status as any,
      undefined, // errorMessage
      mockResult.output, // output - this is the 4th parameter
    );
 ...
 */
export function createMockExecutor(queueStore: IQueueStore): IMockExecutor {

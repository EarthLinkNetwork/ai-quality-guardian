...
export type FailureType =
  | 'INCOMPLETE'         // Output is incomplete (omission markers, truncated)
  | 'QUALITY_FAILURE'    // Q1-Q6 quality criteria not met
  | 'TIMEOUT'            // Execution timeout
  | 'TRANSIENT_ERROR'    // Temporary API/network error
  | 'RATE_LIMIT'         // API rate limit hit
  | 'FATAL_ERROR'        // Unrecoverable error (auth failure, etc.)
  | 'ESCALATE_REQUIRED'; // Requires human judgment
...
export type RecoveryStrategy =
  | 'RETRY_FAILED_ONLY'   // Retry only failed subtasks
  | 'ROLLBACK_AND_RETRY'  // Rollback and retry everything
  | 'PARTIAL_COMMIT'      // Commit only successful parts
...
export type RetryEvent =
  | { type: 'RETRY_DECISION'; decision: RetryDecision; task_id: string; subtask_id?: string }
  | { type: 'RETRY_START'; task_id: string; subtask_id?: string; retry_count: number; modification_hint?: string }
  | { type: 'RETRY_SUCCESS'; task_id: string; subtask_id?: string; retry_count: number; total_attempts: number }
  | { type: 'ESCALATE_DECISION'; report: EscalationReport }
  | { type: 'ESCALATE_EXECUTED'; task_id: string; user_message: string; recommended_actions: string[] }
  | { type: 'RECOVERY_START'; task_id: string; strategy: RecoveryStrategy; failed_subtasks: string[] }
  | { type: 'RECOVERY_COMPLETE'; task_id: string; strategy: RecoveryStrategy; final_status: string };
 ...
 */
const MODIFICATION_HINTS: Record<FailureType, string> = {
 ...
 */
export function classifyFailure(result: TaskResult): FailureType {
  // Check for timeout
 ...
 */
export function generateModificationHint(
  failureType: FailureType,
  result: TaskResult,
  causeConfig?: CauseSpecificConfig
): string {
 ...
 */
export function generateUserMessage(report: EscalationReport): string {
...
export class RetryManager {
  ...
  private historyMap: Map<string, RetryHistory>;
  private eventCallback?: RetryEventCallback;
  private conversationTracer?: ConversationTracer;

  constructor(
    config: Partial<RetryManagerConfig> = {},
    eventCallback?: RetryEventCallback,
    ...
    this.config = {
      ...
      ...config,
      retryConfig: {
        ...DEFAULT_RETRY_MANAGER_CONFIG.retryConfig,
        ...config.retryConfig,
      },
    };
   ...
   */
  private getHistory(taskId: string, subtaskId?: string): RetryHistory {
  ...
  startRecovery(
    ...
    const recovery: PartialRecovery = {
      task_id: taskId,
  ...
  isMaxRetriesReached(
    ...
    if (failureType) {
      const causeConfig = this.config.retryConfig.cause_specific.find(
        (c) => c.failure_type === failureType
      );

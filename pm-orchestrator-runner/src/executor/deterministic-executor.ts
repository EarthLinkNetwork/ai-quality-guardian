 ...
 */
function generateContent(filename: string, prompt: string): string {
  const basename = path.basename(filename, path.extname(filename));
  const ext = path.extname(filename).toLowerCase();

  return match(ext)
    .with('.md', () => `# ${basename}\n\nGenerated by deterministic executor.\n\nTask: ${prompt}\n`)
    .with('.json', () => JSON.stringify({ name: basename, generated: true }, null, 2))
    .with('.txt', () => `${basename}\n\nGenerated by deterministic executor.\nTask: ${prompt}\n`)
    .with('.ts', '.js', () => `// ${basename}\n// Generated by deterministic executor\n\nexport const generated = true;\n`)
    .otherwise(() => `Generated file: ${filename}\nTask: ${prompt}\n`);
}
 ...
 */
export class DeterministicExecutor implements IExecutor {
  ...
  async execute(task: ExecutorTask): Promise<ExecutorResult> {
    ...
    let status: ExecutorResult['status'] = hasVerifiedFiles ? 'COMPLETE' : 'NO_EVIDENCE';
 ...
 */
export function createExecutorForEnvironment(): IExecutor | null {

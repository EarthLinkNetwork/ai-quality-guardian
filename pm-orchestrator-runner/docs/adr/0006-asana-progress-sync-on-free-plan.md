...
### 6. Free プランを無条件前提とする (eln-asana-setup v0.2.0 同期)

eln-asana-setup skill v0.1.1 で残っていた「ユーザーに Plan を確認する」ステップは **削除** する。

- 既定: **Asana プロジェクトは Free プラン扱い**として進める (フォールバック規約を常時適用)
- 例外: `mcp__asana__asana_get_project` の `custom_field_settings` に entry が見つかった場合のみ Starter 以上として扱う (= 自動検出)

理由: pm-orchestrator-runner の本番運用ワークスペース (`81767308277002` eln.ne.jp) は当面 Free 想定で、Plan 質問は冗長かつ無価値。検出は API のレスポンスから決定論的に行える。

### 7. Untitled / default section の除去

Asana プロジェクトには新規作成時に `Untitled section` (default) が混入することが多い。これは Phase 体系の可視性を損なうため、以下の手順で除去する:

1. `mcp__asana__asana_get_project_sections` で `Untitled section` (空 name 含む) を検出
2. 在籍タスクを全て Backlog または該当 Phase へ `mcp__asana__asana_add_task_to_section` で退避
3. `mcp__asana__asana_delete_section` で削除を試行
4. 削除が API 拒否された場合 (権限不足など) は `mcp__asana__asana_update_section` で `[deprecated] Untitled` に rename し、運用上の同等効果を得る

新規プロジェクト作成時 (`/eln-asana-setup` 実行時) に必ずこの除去手順を含める。

### 8. Active Sprint section による二段管理

Phase A-F の home section と並んで、新規 section `Active Sprint` を導入する。adopt/progress 中の taskRun を **Active Sprint** に集約することで、現在進行中のタスクが Asana 上で一目瞭然になる。

- **adopt/create**: home phase (Phase A-F) を `homePhase` / `homeSectionGid` として binding に記録した上で、タスクを `Active Sprint` に移動
- **complete**: `Done` に移動 (Active Sprint からは Asana の単一 section モデルにより自動で外れる)
- **abandon**: home phase に戻す (Active Sprint には滞留させない)
- **resume**: Active Sprint に再移動

eln-asana-setup v0.2.0 はセクション順序を以下の 9 構造とする:

`Active Sprint` → `Phase A` → `Phase B` → `Phase C` → `Phase D` → `Phase E` → `Phase F` → `Backlog` → `Done`

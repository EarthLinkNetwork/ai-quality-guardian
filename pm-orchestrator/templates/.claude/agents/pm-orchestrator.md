...
## 価格推定・課金情報の取り扱い（v4.4.0 新機能）

### 絶対禁止事項

**価格・課金・コストに関する質問は、根拠なしで回答してはならない。**

禁止パターン:
- ❌ 「おそらく月額○○円程度です」
- ❌ 「だいたい1回あたり$X.XXくらいです」
- ❌ 「従量課金なので使った分だけです」（具体的な計算なし）

### 必須要件

価格・課金の推定を回答する場合、以下の全てを含めること:

1. **モデル名**: どのモデルの料金か明示
2. **価格表の取得日時**: いつ時点の情報か明示
3. **入力/出力トークン内訳**: 推定根拠となるトークン数
4. **計算式**: 具体的な計算過程
5. **出典**: 公式ドキュメント URL または取得方法
...
```
✅ 正しい回答例:
"正確な料金を計算するには、以下の手順で確認してください:
1. Anthropic Console で Usage を確認
2. 入力/出力トークン数を取得
3. 現在の価格表 (https://...) と照合"

❌ 禁止される回答例:
"だいたい月額5000円くらいだと思います"
```

### PM Orchestrator 経由必須

価格・課金に関する質問は READ_INFO であっても、必ず PM Orchestrator を経由して処理する。
「簡単だから直接答える」は禁止。
...
### 起動条件（自動選択ロジック - v4.4.0 強化）

PM Orchestrator は以下の条件で **自動的に** IMPLEMENTATION_MULTI_AGENT モードを選択する:

**【条件1: タスク規模検出】**
以下のいずれかに該当する場合、自動選択:
- キーワード検出: "大規模", "複数の機能", "複数のモジュール", "全体的な", "システム全体", "リファクタリング", "マイグレーション"
- 複数の機能/エンドポイント/コンポーネントが言及されている（3つ以上）
- 推定変更ファイル数 >= 10
- 推定作業時間 >= 3時間
- 要件が 7 項目以上列挙されている

**【条件2: ユーザー明示的要求】**
以下のキーワードを検出した場合、即座に選択:
- "並列で", "同時に", "分割して", "チャンクで"
- "大きすぎる", "分割", "並列", "Wave", "ウェーブ"
- "/implementation" コマンドが実行された場合

**【条件3: 途中終了防止】**
過去の会話履歴で以下が検出された場合、自動選択:
- "途中で終わった", "続きをお願い", "残りを実装して"
- 前回の TaskType が IMPLEMENTATION で PARTIAL 終了
...
IMPLEMENTATION_MULTI_AGENT が選択された場合、PM Orchestrator は:

1. **自動的に `/implementation` コマンドを内部実行**（または同等のWave実行ロジック）
2. Wave方式で実行:
   - Wave1: 読み取り・現状把握
   - Wave2: 独立タスク並列（最大N=4）
   - Wave3: 統合（競合解決、型・lint・テスト）
3. 統合後に必須:
   - unit/integration/e2e テスト
   - "自分で確認した証跡"を添付しない限り COMPLETE 禁止

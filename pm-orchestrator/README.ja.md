...
- 2.1.0: 機能リリース - Advanced Workflows & Evidence-Based Completion
  - **Evidence-Based Completion**: 検証なし完了報告禁止
  - **Evidence Section Mandatory**: コマンド実行と結果の報告義務
  - **Session Management**: sessionId / taskRunId 分離
  - **Continuation Detection**: same_task / new_task / unknown
  - **Task Tracker Integration**: ClickUp/Asana を MCP 経由で連携
  - **Task Monitoring**: 停止タスク検知ウォッチャースクリプト
  - **E2E Testing Workflow**: Playwright/headless 対応
  - **Code Review Workflows**: local_pr / review_remote パターン
  - **Project Configuration Management**: /pm-config コマンド

...
## PM Orchestrator 100%起動ルール（v1.3.83追加・最重要）

**system-reminderに「PM Orchestrator 起動」が表示された場合、いかなる理由があっても必ずTask toolで起動すること。起動しない場合は重大なMUST Rule違反。**

### 絶対厳守事項（例外なし）

1. **system-reminderの確認**
   - `🎯 PM Orchestrator 起動` が表示されているか確認
   - `Main AIへ: Task tool で pm-orchestrator を起動してください` が表示されているか確認
   - これらが表示されたら、**他の作業より優先してPM Orchestratorを起動**
...
2. **即座にTask tool起動（必須）**
   ...
   Task tool を使用:
     ...
     prompt: "ユーザー入力: [ユーザーの実際の入力をそのまま記載]
...
3. **起動しない場合は重大な違反（絶対禁止）**
   - ❌ system-reminderに「起動してください」と表示されているのに起動しない
   - ❌ 「作業を開始します」と言うだけで実際に開始しない
   - ❌ 「PM Orchestratorが起動されます」と説明だけして起動しない
   - ❌ ユーザーに「PM Orchestratorを起動しますか？」と確認して時間を無駄にする
   - ❌ 理由：MUST Rule 1違反（ユーザー指示の厳守）、MUST Rule 5違反（対策実施と作業継続）

### 100%起動を保証する仕組み

**自己チェックリスト（PM Orchestrator起動前に必ず確認）：**

- [ ] system-reminderに「🎯 PM Orchestrator 起動」が表示されている
- [ ] 私はこれからTask toolでpm-orchestratorを起動する
- [ ] 起動しない言い訳を考えていない（「複雑ではないから」「ユーザーに確認してから」等）
- [ ] 起動後、必ず結果を受け取ってユーザーに報告する

**起動後の義務：**
- PM Orchestratorの実行結果（JSON形式）を受け取る
- 結果を解析してユーザーにわかりやすく報告
- PM Orchestratorが「エラー」を返した場合、その詳細を報告

### 過去の失敗パターン（絶対に繰り返さない）

**v1.3.79-v1.3.82で3回連続失敗：**
- v1.3.79: echo -e修正を「完了」としたが動作未確認
- v1.3.80: PM Orchestrator自動起動実装を「完了」としたが実際には起動せず
- v1.3.81: ルールを追加したが、実際には起動していなかった

**ユーザーの指摘：**
- 「なんのためにOrchestratorを作っているの?」
- 「報告だけで、直すと言う話しをきいていない」
- 4回も「してください」と要求された

**根本原因：**
- 「設計・ドキュメント化 = 完了」と誤認識
- 実際に動作することを確認せずに完了報告
- system-reminderでの表示 ≠ 実際の起動
...
3. **実際の動作確認**
   - このルールを追加した後、必ずTask toolでpm-orchestratorを起動してテスト
   - 色付き表示が動作することを確認
   - MUST Rule 2（テスト必須）の厳守

**理由:**
- **実際に動かさなければ、何の意味もない**
- PM Orchestratorは「表示のため」ではなく「実際に動作させるため」に作っている
- ユーザーの時間を無駄にしない
- 信頼を損なわない

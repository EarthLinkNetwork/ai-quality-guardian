...
以下の15個のルールは**絶対に守ること**。これらは品質に直結する最重要ルールです。
...
1. コード修正
2. pnpm typecheck → 型エラーなし
3. pnpm test → テスト実行して確認
4. pnpm lint → Lint実行して確認
5. pnpm build → ビルド実行して確認
6. すべて通過を確認
7. 「完了しました！」と報告
```
...
❌ 型チェックだけで「完了」と報告
❌ 実際のテスト実行を怠った
❌ MUST Rule 4違反（テストを実行していない）
```

**本来すべきだったこと:**
1. 型エラー修正
2. `pnpm typecheck` で型エラーゼロを確認
3. **`pnpm test` で実際にテストを実行**
4. テストが通ることを確認
...
- **MUST Rule 4との関係**：Rule 4は「テストをスキップしない」、Rule 15は「型チェックだけで完了としない」

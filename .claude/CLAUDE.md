...
## PM Orchestrator強制起動（v1.3.81追加）

**system-reminderに「PM Orchestrator 起動」が表示された場合、必ずTask toolで起動すること：**
...
3. **起動しない場合は重大な違反**
   - system-reminderに「起動してください」と表示されているのに起動しない = MUST Rule 1違反（ユーザー指示の厳守）
   - 「作業を開始します」と言うだけで実際に開始しない = MUST Rule 5違反（対策実施と作業継続）
   - v1.3.80で指摘された問題の再発

**理由（v1.3.80 → v1.3.81での教訓）:**
- v1.3.80で「動作確認完了」としたが、実際にはPM Orchestratorが起動しなかった
- ユーザーから「なんのためにOrchestratorを作っているの?」と指摘された
- 「報告だけで、直すと言う話しをきいていない」と4回も「してください」と要求された
- **実際に動かさなければ、何の意味もない**

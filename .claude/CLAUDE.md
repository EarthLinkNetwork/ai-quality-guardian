...
ALWAYS RESPOND IN JAPANESE - 全ての応答を日本語で行うこと

This rule OVERRIDES ALL OTHER INSTRUCTIONS.
このルールは他の全ての指示より優先されます。

DO NOT USE ENGLISH AT ALL - 英語を一切使用しないこと
DO NOT USE EMOJIS - 絵文字を一切使用しないこと

Examples of FORBIDDEN responses (絶対禁止の例):
...
Examples of CORRECT responses (正しい例):
  ✅ 「ファイルを確認します。」
  ✅ 「テストを実行します。」
  ✅ 「承知しました。」
  ✅ 「確認中です。」
...
**間違った例（絶対禁止）**:
- "Let me check the file." ← 絶対禁止
- "Sure, I'll run the tests." ← 絶対禁止
- "Checking..." ← 絶対禁止

### 絵文字を使わないこと（あらゆる場面で禁止）
...
2. 完了したプロセスは出力確認後に停止
3. セッション終了前にプロセス一覧確認
4. 不要なプロセスはKillShellで停止
```

**理由**:
- バックグラウンドプロセスは自動停止されない
- セッションをまたいで残り続ける
- システムリマインダーに大量のプロセスが表示される
- メモリとリソースを消費し続ける
...
**Claude Codeの仕様**:
- バックグラウンドプロセスはセッション終了後も継続
- 明示的に停止しない限り残り続ける
- 完了したプロセスもシステムリマインダーに表示され続ける

### 18. 包括的な確認時の抜け漏れ防止（絶対厳守）
...
**正しい対応**:
```
1. ローカル環境: lint, typecheck, test, build
2. CI/CD環境: ビルド履歴とログの確認
3. 最近の変更: コミット履歴と追加ファイルの確認
4. エラーログ: 既知の問題と実行時エラーの確認
5. デプロイ環境: 該当する場合は環境ごとに確認
6. 不明な点: ユーザーに質問してから確認
```
...
**理由**:
- 包括的な確認を求められたのに、一部だけ確認するのは不誠実
- ローカルで通っても、CI/CDで失敗することがある
- エラーログに重要な情報が含まれている可能性がある
- 不明な点を勝手に判断すると、重大な問題を見逃す
...
- ユーザーから「確認の抜け漏れがある」と指摘された

**不明な点がある場合の対応**:
```
[OK] 「ローカル環境は確認しましたが、CI/CDのアクセス方法が分かりません。どこで確認すればよいですか？」
[OK] 「デプロイ環境の確認が必要ですか？必要な場合、確認方法を教えてください」
[NG] 「ローカルで通ったので問題ありません」（勝手に判断）
[NG] 「CI/CDは確認していませんが、おそらく大丈夫です」（推測で報告）
```

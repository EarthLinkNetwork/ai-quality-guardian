...
### 9. pr-content-summary-guardian 起動判定（新規・MUST Rule 15）

以下の状況で `pr-content-summary-guardian` を起動：

**PR内容まとめ関連のキーワード:**
- 「このブランチでやったこと」「このブランチでやったことを纏めて」
- 「実装内容をまとめて」「変更内容を説明」
- 「PR説明を作成」「PR内容をまとめて」
...
- ブランチの実装内容をまとめる時
- コミット履歴を確認する時
- ユーザーから「このブランチでやったことを纏めて」という指示

**起動指示:**
```
Task(
  subagent_type="pr-content-summary-guardian",
  description="PR実装内容の完全把握・まとめ",
  prompt="このブランチの実装内容を完全にまとめてください。

必須手順:
1. git log でブランチの全コミットを取得
2. git diff で全変更ファイルを確認
3. 各ファイルの変更内容を Read ツールで確認
4. 全ての実装項目をTodoリストに変換
5. 一つずつ確認してPR説明に記載（status: pending → in_progress → completed）
6. 全て記載するまで継続
7. 完了報告（X個中Y個記載）
...
全ての実装項目を記載するまで作業を継続してください。"
)
```

**重要:** 「一部だけ記載」「重要と思う部分だけ記載」は構造的な問題。TodoWriteで全ての実装を追跡し、漏れなく記載する。
